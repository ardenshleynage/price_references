                    ...
                    <tbody>
                        @foreach ($results['products'] as $product)
                            @php
                                ...
                                $postScriptum = isset($product->post_scriptum)
                                    ? addslashes($product->post_scriptum)
                                    : 'Aucun';
                                $detailedPrice = isset($product->detailed_price)
                                    ? addslashes($product->detailed_price)
                                    : 'Aucun';
                                $branchName = isset($product->branch->branche_name)
                                    ? addslashes($product->branch->branche_name)
                                    : 'Aucun';
                                $categoryName = isset($product->category->category_name)
                                    ? addslashes($product->category->category_name)
                                    : 'Aucun';
                            @endphp
