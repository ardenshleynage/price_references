                    ...
                    <div class="table-data">
                        <div class="order">
                            <table>
                                ...
                                <tbody>
                                    @php
                                        $lastConnect = isset($user->last_time_connect)
                                            ? addslashes($user->last_time_connect)
                                            : 'Jamais';
                                    @endphp
                                    @foreach ($results['users'] as $user)
                                        <tr onclick="openUserModal('{{ $user->id }}', '{{ addslashes($user->username) }}', '{{ $user->email ?? 'N/A' }}', '{{ $user->role }}', '{{ $user->status }}', '{{ $lastConnect }}', '{{ $user->created_at }}', '{{ $user->updated_at }}')"
