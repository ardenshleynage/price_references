...
class SuperAdminController extends Controller
{
    ...
    public function createUsersAdmins(Request $request): RedirectResponse
    {
        ...
        if ($request->role == 1) {
            ...
            $existingSuperAdmin = User::where('role', 1)->first();
    ...
    public function updateUser(Request $request): RedirectResponse
    {
        ...
        // Validation conditionnelle - seulement si le champ est présent
        if ($request->filled('username')) {
            $validationRules['username'] = 'string|max:255|min:2|unique:users,username,'.$id;
            $validationMessages['username.string'] = "Le nom d'utilisateur doit être une chaîne de caractères.";
            $validationMessages['username.max'] = "Le nom d'utilisateur doit contenir au maximum 255 caractères.";
            $validationMessages['username.min'] = "Le nom d'utilisateur doit contenir au moins 2 caractères.";
            $validationMessages['username.unique'] = "Ce nom d'utilisateur existe déjà.";
        }

        if ($request->filled('email')) {
            $validationRules['email'] = 'string|email|max:255|unique:users,email,'.$id.'|ends_with:gmail.com';
            $validationMessages['email.string'] = "L'adresse email doit être une chaîne de caractères.";
            $validationMessages['email.email'] = "L'adresse email doit être valide.";
            $validationMessages['email.max'] = "L'adresse email doit contenir au maximum 255 caractères.";
            $validationMessages['email.unique'] = 'Cette adresse email existe déjà.';
            $validationMessages['email.ends_with'] = "L'adresse email doit être une adresse Gmail (@gmail.com).";
        }

        if ($request->filled('role')) {
            $validationRules['role'] = 'integer|in:2,3';
            $validationMessages['role.integer'] = 'Le rôle doit être un nombre entier.';
            $validationMessages['role.in'] = 'Le rôle doit être admin (2) ou utilisateur (3).';
        }
    ...
    public function updateUserFromSearch(Request $request): RedirectResponse
    {
        ...
        // Validation conditionnelle - seulement si le champ est présent
        if ($request->filled('username')) {
            $validationRules['username'] = 'string|unique:users,username,'.$id.'|max:255|min:2';
            $validationMessages['username.string'] = "Le nom d'utilisateur doit être une chaîne de caractères.";
            $validationMessages['username.max'] = "Le nom d'utilisateur doit contenir au maximum 255 caractères.";
            $validationMessages['username.min'] = "Le nom d'utilisateur doit contenir au moins 2 caractères.";
            $validationMessages['username.unique'] = "Ce nom d'utilisateur existe déjà.";
        ...
        if ($request->filled('role')) {
            $validationRules['role'] = 'integer|in:2,3';
            $validationMessages['role.integer'] = 'Le rôle doit être un nombre.';
            $validationMessages['role.in'] = 'Le rôle doit être admin (2) ou utilisateur (3).';
        }

        // Au moins un champ doit être présent
        if (empty($validationRules)) {
            return back()->withErrors(['error' => 'Aucune donnée à modifier.']);
        }
